...
export const useStore = create<State & Actions>((set) => ({
  ...
  restartGame: (mode: State['mode']) => set(() => {
    const citiesToPlay = getCitiesToPlay(mode)
    return {
      ...initialState,
      mode,
  ...
  endTurn: (clickedCoordinate?: Coordinates) => set((state) => {
    const {
      playedCities,
      citiesToPlay,
      currentCity,
  ...
  saveScore: (name: string) => set(({ scores, score, mode, playedCities }) => {
  ...
  nextCity: () => set((state) => (state.pause && state.round !== CITIES_PER_GAME) ? ({ pause: null, isRunning: true, round: state.round + 1 }) : state),
