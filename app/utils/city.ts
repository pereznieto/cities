...
export const getRandomCity =
  (minimumPopulation: number, maximumPopulation = 100000000) =>
  (playedIds: readonly number[]): City => {
    const unplayedCities = getCities(allCities).filter(
      ({ id, population }) =>
        playedIds.indexOf(id) === -1 && population > minimumPopulation && population < maximumPopulation,
    )
...
export const getCities = (rawCities: readonly RawCity[]): readonly City[] =>
  rawCities.map(({ n: name, c: country, p: population, lt: latitude, lg: longitude }, index) => ({
    name,
    latitude,
    longitude,
    country,
...
export const getDisplayName = ({ name, country }: City, difficulty: Difficulty): string =>
  difficulty === Difficulty.Easy || isCityNameDuplicate(name) ? `${name}, ${country}` : name

export const isCityNameDuplicate = (cityName: string): boolean =>
  getCities(allCities).reduce<readonly City[]>(
    (accumulator, current) => (current.name === cityName ? [...accumulator, current] : accumulator),
    [],
  ).length > 1
